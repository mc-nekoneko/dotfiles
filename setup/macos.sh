...
if [ "$1" = "--force" ]; then
    ...
    echo "The following macOS settings will be applied:"
    echo "  - Finder: show all files, extensions, path bar, status bar"
    echo "  - Keyboard: fast key repeat"
    echo "  - Scroll: natural scrolling ON"
    echo "  - Dock: autohide, remove default apps, no recents"
    echo "  - Bluetooth: higher audio quality"
    echo "  - Photos: disable auto-launch on USB connect"
    echo ""
